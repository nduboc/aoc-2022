...
long compute_rec_sizes(Dir *tree, long limit, long *sum) {
    long recSize = tree->ownSize;
    for (Dir *subdir = tree->subdirs; subdir != NULL; subdir = subdir->siblings) {
        recSize += compute_rec_sizes(subdir, limit, sum);
    }
    if (recSize <= limit) {
        *sum += recSize;
    }
...
int main() {
    ...
    long result = 0;
    long root_size = compute_rec_sizes(root, 100000, &result);
    printf("Root size: %ld\n", root_size);
    printf("Result: %ld, confirmed: 1243729\n", result);
}
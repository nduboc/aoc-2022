...
def monkey(op, test, if_true, if_false):
    def turn(i, monkey_lists):
        ...
        if test(worry):
            ...
            monkey_lists[if_true].append(worry % reducer)
        else:
            debug(f"      worry {worry} send to {if_false}")
            monkey_lists[if_false].append(worry % reducer)
    return turn
...
print(monkey_counts)
monkey_counts.sort()
print(f"{monkey_counts[-2] * monkey_counts[-1]}, confirmed 54832778815")
